...
const Chats = () => {
  const {selectedChat,setSelectedChat,selectedGroup,
    setSelectedGroup} = useContext(DataContext)
  ...
  return (
    <div className="flex  flex-col gap-4">
      ...
      <div className="flex items-center justify-center gap-8 ">
        <div onClick={()=>setIsPeoplesList(true)} className="relative  cursor-pointer">
          <h6 className="font-semibold text-md ">People</h6>
          {isPeoplesList &&<div className="absolute  -bottom-4 color transition-all">________</div>}
        </div>
        <div onClick={()=>setIsPeoplesList(false)} className="relative cursor-pointer">
      ...
      {
       isPeoplesList ? isFetching  ? "" :  personalChats.map(chat =>{
        ...
        )}) : gettingData ? "" : groupsChats.map(chat => {
          let latestMessageTime = ""
          if(chat.latestMessage){
           latestMessageTime =new Date(chat?.latestMessage?.createdAt)
           .toLocaleTimeString(navigator.language, {hour: '2-digit', minute:'2-digit'})
