...
const AddGroupMembers = () => {
    ...
    const { data, isLoading } = useSearchUsersQuery(search,{skip});
    const handleSearch = () => {
      if (search !== "") {
        setSkip(false)
        if(isLoading === false){
    ...
    <div className="flex  flex-col gap-4">
      <div className="p-2 mt-2 items-center gap-2 shadow-xl bg-white rounded-lg flex">
        ...
        <div>
            <button
              ...
              className="px-2 py-1 rounded-lg bg text-white"
            >
              Go
            </button>
          </div>
      ...
      <div className="flex flex-col gap-2 overflow-hidden overflow-y-scroll h-[25.5pc]">
      {
          ...
          searchResults?.map(usr => (
            <div onClick={()=>handleUserAdd(usr)} className="flex items-center  hover:bg-gray-100 cursor-pointer gap-1 h-[4.5rem]  px-2 shadow-xl bg-white rounded-xl">
