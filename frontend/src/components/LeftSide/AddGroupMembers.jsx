import React, { useContext, useState } from 'react'
import { HiOutlineArrowLeft } from 'react-icons/hi'
import { DataContext } from '../../context/DataProvider'
import { BiSearch } from "react-icons/bi";
import { useTransition,animated } from "react-spring";
import { BsArrowRightCircleFill } from "react-icons/bs";
import { FaCircle } from "react-icons/fa"; 
import CreateGroup from './CreateGroup';
const AddGroupMembers = () => {
    const dummyProfile = "data:image/jpeg;base64,";

    const {setAddGroupMembersPageOpen,createGroupPageOpen,setCreateGroupPageOpen} = useContext(DataContext)
    const [inputActive, setInputActive] = useState(false);
    const [selected, setSelected] = useState(true)
    const transition = useTransition(createGroupPageOpen,{
      from:{x:-100 ,y:0 ,opacity:0},
      enter:{x:0 ,y:0 ,opacity:1},
      leave:{x:-100 ,y:0 ,opacity:0},
     })
  return (
    <div className='relative'>
    <div className='bg h-[4.5rem]  flex gap-2 items-end'>
       <div className='flex gap-4 p-4 '>
       <HiOutlineArrowLeft 
       onClick={()=>setAddGroupMembersPageOpen(false)}
            size={27}
            className="transition-all  cursor-pointer"
            color="white"
          />
          <h6 className='font-semibold text-lg text-white'>Add group participants</h6>
       </div>
    </div>
    <div className="flex  flex-col gap-4">
      <div className="p-2 mt-2 items-center gap-2 shadow-xl bg-white rounded-lg flex">
        {!inputActive ? (
          <BiSearch size={27} className="transition-all  color"  />
        ) : (
          <HiOutlineArrowLeft
          onClick={() => setInputActive(false)}
            size={27}
            className="transition-all  cursor-pointer color"
           
          />
        )}
        <input
          onBlur={() => setInputActive(false)}
          onClick={() => setInputActive(true)}
          type="text"
          placeholder="Find an angel..."
          className=" w-full  placeholder:text-purple-300 border-none rounded-lg outline-none"
        />
      </div>
      <div className="flex flex-col gap-2 overflow-hidden overflow-y-scroll h-[25.5pc]">
        <div className="flex items-center  hover:bg-gray-100 cursor-pointer gap-1 h-[4.5rem]  px-2 shadow-xl bg-white rounded-xl">
          <div className="w-14  h-14 py- flex items-center justify-center">
            <img
              className="w-12 h-12 hover:w-14
                hover:h-14
                transition-all rounded-full shadow-xl"
              src={dummyProfile}
              alt="profile"
            /> 
          </div>
          <div className="flex flex-col grow gap-0  justify-end">
            <h6 className="font-semibold leading-7 text-base">My angel</h6>
            <p className="font-normal text-sm ">shanideee......</p>
          </div>
          <div className="relative flex flex-col mt-2 items-center justify-end gap-1">
            <FaCircle size={20} className="color justify-center"/>
            <p className="font-normal text-sm">12:01 Pm</p>
            <div className="absolute top-[2px] right-[1.6rem]">
              <p className="text-white text-xs font-semibold ">2</p>
            </div>
          </div>
        </div>
        <div 
        className={`flex items-center 
          ${selected ? "bg " : "bg-white hover:bg-gray-100"} cursor-pointer gap-1 h-[4.5rem]
           px-2 shadow-xl  rounded-xl`}>
          <div className="w-14  h-14 py- flex items-center justify-center">
            <img
              className="w-12 h-12 hover:w-14
                hover:h-14
                transition-all rounded-full shadow-xl"
              src={dummyProfile}
              alt="profile"
            />
          </div>
          <div className="flex flex-col grow gap-0  justify-end">
            <h6 className={`font-semibold leading-7 ${selected ? "text-white" : ""} text-base`}>My angel</h6>
            <p className={`font-normal text-sm ${selected ? "text-white" : ""}`}>shanideee......</p>
          </div>
          <div className="relative flex flex-col mt-2 items-center justify-end gap-1">
            <FaCircle size={20} className={`${selected ? "text-white" : "color"} justify-center`}/>
            <p className={`font-normal text-sm ${selected ? "text-white" : ""}`}>12:01 Pm</p>
            <div className="absolute top-[2px] right-[1.6rem]">
              <p className={`${selected ? "text-black" : "text-white"} text-xs font-semibold `}>2</p>
            </div>
          </div>
        </div>
        <div className="flex items-center  hover:bg-gray-100 cursor-pointer gap-1 h-[4.5rem]  px-2 shadow-xl bg-white rounded-xl">
          <div className="w-14  h-14 py- flex items-center justify-center">
            <img
              className="w-12 h-12 hover:w-14
                hover:h-14
                transition-all rounded-full shadow-xl"
              src={dummyProfile}
              alt="profile"
            />
          </div>
          <div className="flex flex-col grow gap-0  justify-end">
            <h6 className="font-semibold leading-7 text-base">My angel</h6>
            <p className="font-normal text-sm ">shanideee......</p>
          </div>
          <div className="relative flex flex-col mt-2 items-center justify-end gap-1">
            <FaCircle size={20} className="color justify-center"/>
            <p className="font-normal text-sm">12:01 Pm</p>
            <div className="absolute top-[2px] right-[1.6rem]">
              <p className="text-white text-xs font-semibold ">2</p>
            </div>
          </div>
        </div>
        <div className="flex items-center  hover:bg-gray-100 cursor-pointer gap-1 h-[4.5rem]  px-2 shadow-xl bg-white rounded-xl">
          <div className="w-14  h-14 py- flex items-center justify-center">
            <img
              className="w-12 h-12 hover:w-14
                hover:h-14
                transition-all rounded-full shadow-xl"
              src={dummyProfile}
              alt="profile"
            />
          </div>
          <div className="flex flex-col grow gap-0  justify-end">
            <h6 className="font-semibold leading-7 text-base">My angel</h6>
            <p className="font-normal text-sm ">shanideee......</p>
          </div>
          <div className="relative flex flex-col mt-2 items-center justify-end gap-1">
            <FaCircle size={20} className="color justify-center"/>
            <p className="font-normal text-sm">12:01 Pm</p>
            <div className="absolute top-[2px] right-[1.6rem]">
              <p className="text-white text-xs font-semibold ">2</p>
            </div>
          </div>
        </div>
       
        <div className="flex items-center  hover:bg-gray-100 cursor-pointer gap-1 h-[4.5rem]  px-2 shadow-xl bg-white rounded-xl">
          <div className="w-14  h-14 py- flex items-center justify-center">
            <img
              className="w-12 h-12 hover:w-14
                hover:h-14
                transition-all rounded-full shadow-xl"
              src={dummyProfile}
              alt="profile"
            />
          </div>
          <div className="flex flex-col grow gap-0  justify-end">
            <h6 className="font-semibold leading-7 text-base">My angel</h6>
            <p className="font-normal text-sm ">shanideee......</p>
          </div>
          <div className="relative flex flex-col mt-2 items-center justify-end gap-1">
            <FaCircle size={20} className="color justify-center"/>
            <p className="font-normal text-sm">12:01 Pm</p>
            <div className="absolute top-[2px] right-[1.6rem]">
              <p className="text-white text-xs font-semibold ">2</p>
            </div>
          </div>
        </div>
       
        <div className="flex items-center  hover:bg-gray-100 cursor-pointer gap-1 h-[4.5rem]  px-2 shadow-xl bg-white rounded-xl">
          <div className="w-14  h-14 py- flex items-center justify-center">
            <img
              className="w-12 h-12 hover:w-14
                hover:h-14
                transition-all rounded-full shadow-xl"
              src={dummyProfile}
              alt="profile"
            />
          </div>
          <div className="flex flex-col grow gap-0  justify-end">
            <h6 className="font-semibold leading-7 text-base">My angel</h6>
            <p className="font-normal text-sm ">shanideee......</p>
          </div>
          <div className="relative flex flex-col mt-2 items-center justify-end gap-1">
            <FaCircle size={20} className="color justify-center"/>
            <p className="font-normal text-sm">12:01 Pm</p>
            <div className="absolute top-[2px] right-[1.6rem]">
              <p className="text-white text-xs font-semibold ">2</p>
            </div>
          </div>
        </div>
        </div>
      </div>
      <div className='h-20 bg-white w-full flex items-center justify-center absolute bottom-0'>
        <div className=''>
          <BsArrowRightCircleFill onClick={()=>setCreateGroupPageOpen(true)} size={34} className="cursor-pointer color"/>
        </div>
      </div>
      {
        transition((style,item)=>
          item ? <animated.div style={style}  className={`transition-all 
          z-10 bg-white 
          absolute inset-0`}>
            <CreateGroup/>
          </animated.div>  : ""
        )
      }
    </div>
  )
}

export default AddGroupMembers